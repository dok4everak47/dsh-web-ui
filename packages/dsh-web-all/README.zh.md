...
- **侧边栏折叠记忆**：compat 桥接层会把左侧侧边栏的折叠/展开状态记到 localStorage，刷新页面后恢复；这是因为 dsh shell 自身只把折叠状态放在 React 内存里。存储键名为 `dsh:sidebar-collapsed`；清除站点数据后，下次加载侧边栏会回到 shell 默认状态（展开）。
