...
- **Sidebar fold memory**: the compat bridge remembers the left sidebar collapsed/expanded state in localStorage and restores it the moment the column mounts after a reload (transition-suppressed for that one tick, so the sidebar appears already in place instead of animating closed), because the dsh shell itself keeps the fold state in a transient React store only. The storage key is `dsh:sidebar-collapsed`; clearing site data returns the sidebar to the shell default (expanded) on the next load. Set localStorage `dsh:sidebar-memory` to `off` before load to disable the feature.
