...
- **分页列表**：轮次每页显示 5 条，面板底部提供页码输入跳转（以及
  ...
  拉取。历史拉取带滚动静默闸门：聊天滚动区正在滚动时暂停拉取，停止
  约 0.35 秒后才继续下一页，避免历史前置插入与滚动中的锚定/贴底跟随
  竞争导致页面跳动。页码输入跳转在历史加载期间也始终可见（禁用、
  总数显示为“…”）；不足 5 条的末页会留出空白占位，使列表高度跨页
  保持稳定。
...
- 只能滚动到**已加载**的历史页——DSH Web 客户端按需分页加载历史，
  面板打开时会把全部历史翻页拉入共享窗口（受滚动静默闸门节拍），
  之后聊天滚动区也持有全部历史。
