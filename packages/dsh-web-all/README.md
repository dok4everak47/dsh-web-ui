...
- **Sidebar fold memory**: the compat bridge remembers the left sidebar collapsed/expanded state in localStorage and restores it within the first frame after the column mounts after a reload, because the dsh shell itself keeps the fold state in a transient React store only. The shell animates the frame grid, resize handle, and the sidebar's own inner content (header actions fade, search narrows) on every fold, and that inner content commits in a later render than the frame -- so a one-tick frame suppression still let the close animation show through. The restore instead puts a `data-dsh-sidebar-boot` attribute on `<html>` that disables transitions across the whole sidebar subtree until the first user interaction (pointer / key / wheel / touch) or a 4 s cap, so a reload of a collapsed session stays collapsed with no visible animation; once you interact, normal transitions return and hand-toggling animates as usual. The storage key is `dsh:sidebar-collapsed`; clearing site data returns the sidebar to the shell default (expanded) on the next load. Set localStorage `dsh:sidebar-memory` to `off` before load to disable the feature.
